...
import com.lfsolutions.retail.databinding.FragmentCreateDriverMemoBinding
...
class CreateDriverMemoFragment : Fragment() {

    private lateinit var itemSwipeHelper: ItemTouchHelper
    private var customer: Customer? = null
    private var memo: DriverMemo? = null
    private lateinit var binding: FragmentCreateDriverMemoBinding
    private lateinit var adapter: DriverMemoProductAdapter
    private val args by navArgs<CreateDriverMemoFragmentArgs>()
        ...
        if (::binding.isInitialized.not()) {
            ...
            Main.app.getDriverMemo().driverMemo.creationTime =
                DateTime.getCurrentDateTime(DateTime.ServerDateTimeFormat).replace(" ", "T")
                    .plus("Z")
            binding = FragmentCreateDriverMemoBinding.inflate(inflater, container, false)
            setProductAdapter()
        }
        return binding.root
