...
class InComingEquipmentListFragment : Fragment() {
    ...
    private lateinit var mAdapter: InComingStockEquipmentAdapter
    ...
    private fun updateEquipmentListView(products: List<Product>) {
        mAdapter = InComingStockEquipmentAdapter(products)
        mAdapter.setListener(object : InComingStockEquipmentAdapter.OnEquipmentClickListener {
            override fun onEquipmentClick(product: Product) {
                findNavController()
                    .navigate(
                        R.id.navigation_incoming_stock_add_product,
